...
$url          = $conn.Default.Url
$clientId     = $conn.Default.ClientId
$clientSecret = $conn.Default.ClientSecret
$solution     = $conn.Default.Solution

if (-not $url) { Write-Error "connections.json must contain a 'Default.Url' value."; exit 1 }
