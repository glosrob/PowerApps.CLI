...
public class ConstantsFilter : IConstantsFilter
{
    ...
    private List<AttributeSchema> FilterAttributeList(List<AttributeSchema> attributes, ConstantsConfig config)
    {
        ...
        // Skip virtual fields — three signals:
        //   AttributeOf set: lookup companion fields generated by Dataverse (e.g. createdbyname, owneridname)
        //   AttributeType "Virtual": image URLs, rollup proxies etc. that have no AttributeOf but are equally unusable in a ColumnSet
        //   Money + _base suffix: base-currency companion fields (e.g. xrt_amount_base). The SDK does not
        //     reliably populate AttributeOf for these, so they require a dedicated pattern check.
        // Note: MultiSelectPicklist fields report as Virtual in the raw SDK enum but MetadataMapper corrects
        // them to "MultiSelectPicklist" before they reach here, so they are not affected by this filter.
        if (config.SkipVirtualFields)
        {
            filteredAttributes = filteredAttributes.Where(a =>
                ...
                !IsCurrencyBaseField(a));
        }

        return filteredAttributes.ToList();
    }

    private static bool IsCurrencyBaseField(AttributeSchema a) =>
        string.Equals(a.AttributeType, "Money", StringComparison.OrdinalIgnoreCase) &&
        a.LogicalName.EndsWith("_base", StringComparison.OrdinalIgnoreCase);
}
