...
public class ConstantsFilter : IConstantsFilter
{
    ...
    private List<AttributeSchema> FilterAttributeList(List<AttributeSchema> attributes, ConstantsConfig config)
    {
        ...
        // Skip virtual fields — two signals:
        //   AttributeOf set: lookup companion fields generated by Dataverse (e.g. createdbyname, owneridname)
        //   AttributeType "Virtual" with no OptionSet: image URLs, rollup proxies etc. that have no AttributeOf
        //     and are equally unusable in a ColumnSet.
        //   NOTE: multi-select choice columns (MultiSelectPicklistAttributeMetadata) also have AttributeType
        //     "Virtual" in the SDK but DO have an OptionSet — they are real, usable columns and must be preserved.
        if (config.SkipVirtualFields)
        {
            filteredAttributes = filteredAttributes.Where(a =>
                string.IsNullOrEmpty(a.AttributeOf) &&
                !(string.Equals(a.AttributeType, "Virtual", StringComparison.OrdinalIgnoreCase) && a.OptionSet == null));
        }
